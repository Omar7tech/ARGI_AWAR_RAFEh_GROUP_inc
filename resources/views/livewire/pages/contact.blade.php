@section('meta_description', isset($meta_description) ? $meta_description : null)
<section
    class="py-12 md:py-16 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 font-inter min-h-screen transition-colors duration-300">
    ...
    <div class="relative container mx-auto px-4 max-w-3xl">
                ...
                <div class="flex items-center justify-center gap-3 mt-6">
                    @foreach ($contactSettings->social_media as $s)
                        <a href="{{ $s['url'] }}" target="_blank" rel="noopener noreferrer"
                            ...
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-cyan-500/10 to-teal-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-full">
                            </div>
                        </a>
                    @endforeach
                </div>
        ...
        <div class="space-y-6">
                    ...
                    <div class="flex items-center gap-3 mb-5 pb-4 border-b border-gray-200 dark:border-gray-700">
                        <div class="w-10 h-10 rounded-lg bg-cyan-100 dark:bg-cyan-900/30 flex items-center justify-center flex-shrink-0">
                            <svg class="w-5 h-5 text-cyan-600 dark:text-cyan-400" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.772-1.548a1 1 0 011.06-.54l4.435.74A1 1 0 0118 16.847V17a1 1 0 01-1 1h-2C7.22 18 2 12.78 2 6V3z" />
                            </svg>
                        </div>
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white">Direct Lines</h2>
                    </div>
                    <div class="space-y-3">
                            ...
                            <a href="tel:{{ $p['phone_number'] }}"
                               class="flex items-center gap-3 p-3 rounded-lg hover:bg-cyan-50 dark:hover:bg-gray-700 transition-all group">
                                <div class="flex-shrink-0">
                                    <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 w-24 text-left">{{ $p['label'] }}</p>
                                </div>
                                <p class="text-base font-semibold text-gray-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors flex-1 text-center">{{ $p['phone_number'] }}</p>
                                <svg class="w-4 h-4 text-gray-400 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-all flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @foreach ($contactSettings->emails as $e)
                            <a href="mailto:{{ $e['email'] }}"
                               class="flex items-center gap-3 p-3 rounded-lg hover:bg-teal-50 dark:hover:bg-gray-700 transition-all group">
                                <div class="flex-shrink-0">
                                    <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 w-24 text-left">{{ $e['label'] }}</p>
                                </div>
                                <p class="text-base font-semibold text-gray-900 dark:text-white group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors flex-1 text-center truncate">{{ $e['email'] }}</p>
                                <svg class="w-4 h-4 text-gray-400 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-all flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                                </svg>
                            </a>
                        @endforeach
            ...
            {{-- Locations --}}
            @if ($contactSettings->locations && count($contactSettings->locations) > 0)
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 border border-gray-200 dark:border-gray-700">
                    <div class="flex items-center gap-3 mb-5 pb-4 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white">Locations</h2>
                    </div>
                    <div class="space-y-3">
                        @foreach ($contactSettings->locations as $l)
                            <p class="text-base font-semibold text-gray-900 dark:text-white">{{ $l['name'] ?? '' }}</p>
                        @endforeach
                    </div>
                </div>
            @endif
